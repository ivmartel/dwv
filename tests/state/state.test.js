...
import v01Ellipse from './v0.1/state-ellipse.json?raw';
import v01LineMulti from './v0.1/state-line_multi-slice.json?raw';
import v01Line from './v0.1/state-line.json?raw';
import v01Protractor from './v0.1/state-protractor.json?raw';
import v01Rectangle from './v0.1/state-rectangle.json?raw';
import v01Roi from './v0.1/state-roi.json?raw';

import v02Arrow from './v0.2/state-arrow.json?raw';
import v02Ellipse from './v0.2/state-ellipse.json?raw';
import v02Hand from './v0.2/state-hand.json?raw';
import v02Protractor from './v0.2/state-protractor.json?raw';
import v02Rectangle from './v0.2/state-rectangle.json?raw';
import v02Roi from './v0.2/state-roi.json?raw';
import v02RulerMulti from './v0.2/state-ruler_multi-slice.json?raw';
import v02Ruler from './v0.2/state-ruler.json?raw';

import v03Arrow from './v0.3/state-arrow.json?raw';
import v03Ellipse from './v0.3/state-ellipse.json?raw';
import v03Hand from './v0.3/state-hand.json?raw';
import v03Protractor from './v0.3/state-protractor.json?raw';
import v03Rectangle from './v0.3/state-rectangle.json?raw';
import v03Roi from './v0.3/state-roi.json?raw';
import v03RulerMulti from './v0.3/state-ruler_multi-slice.json?raw';
import v03Ruler from './v0.3/state-ruler.json?raw';

import v04Arrow from './v0.4/state-arrow.json?raw';
import v04Ellipse from './v0.4/state-ellipse.json?raw';
import v04Hand from './v0.4/state-hand.json?raw';
import v04Protractor from './v0.4/state-protractor.json?raw';
import v04Rectangle from './v0.4/state-rectangle.json?raw';
import v04Roi from './v0.4/state-roi.json?raw';
import v04RulerMulti from './v0.4/state-ruler_multi-slice.json?raw';
import v04Ruler from './v0.4/state-ruler.json?raw';

import v05Arrow from './v0.5/state-arrow.json?raw';
import v05Ellipse from './v0.5/state-ellipse.json?raw';
import v05Hand from './v0.5/state-hand.json?raw';
import v05Protractor from './v0.5/state-protractor.json?raw';
import v05Rectangle from './v0.5/state-rectangle.json?raw';
import v05Roi from './v0.5/state-roi.json?raw';
import v05RulerMulti from './v0.5/state-ruler_multi-slice.json?raw';
import v05Ruler from './v0.5/state-ruler.json?raw';
...
describe('state', () => {
   ...
   */
  function testState(data, version, type) {
    // read state
    const state = new State();
    const jsonData = state.fromJSON(data);
    // check drawings values
    checkDrawings(
      jsonData.drawings, jsonData.drawingsDetails, version, type);
    ...
    checkStateHeader(jsonData, version);
  }
   ...
   * @param {string} type The type of drawing.
   */
  function checkDrawings(drawings, details, version, type) {
    // first level: layer
    ...
    if (drawings.children.length === 5 &&
      ...
      checkRulerDrawings(drawings.children, details, version);
    } else if (drawings.children.length === 1) {

      const layerKid = drawings.children[0];
      assert.equal(layerKid.className, 'Group',
        'Layer first level is a group.');
      assert.equal(
        layerKid.attrs.name,
        'position-group',
      ...
      if (type === 'arrow') {
        checkArrowDrawing(posGroupKid, details, version);
        ...
        checkRulerDrawing(
          posGroupKid, details, version, refRuler);
        ...
        checkRulerDrawing(
          posGroupKid, details, version, refLine);
      } else if (type === 'roi') {
        checkRoiDrawing(posGroupKid, details, version);
      } else if (type === 'hand') {
        checkHandDrawing(posGroupKid, details, version);
      } else if (type === 'ellipse') {
        checkEllipseDrawing(posGroupKid, details, version);
      } else if (type === 'protractor') {
        checkProtractorDrawing(posGroupKid, details, version);
      } else if (type === 'rectangle') {
        checkRectangleDrawing(posGroupKid, details, version);
      } else {
   ...
   * @param {string} version The state format version.
   */
  function checkArrowDrawing(posGroupKid, details, version) {
    // check group
    assert.equal(
  ...
  function checkRulerDrawing(
    posGroupKid, details, version, ref) {
    // check group
    assert.equal(
  ...
  function checkRulerDrawings(layerKids, details, version) {
    ...
    for (let i = 0; i < ndraws; ++i) {
      ...
      checkRulerDrawing(
        posGroupKid, details, version, refRulers[i]);
    }
  }
   ...
   * @param {string} version The state format version.
   */
  function checkRoiDrawing(posGroupKid, details, version) {
    // check group
    assert.equal(
   ...
   * @param {string} version The state format version.
   */
  function checkHandDrawing(posGroupKid, details, version) {
    // check group
    assert.equal(
      posGroupKid.attrs.name,
      'freeHand-group',
    ...
    for (let i = 0; i < posGroupKid.children.length; ++i) {
      ...
      if (shapeGroupKid.attrs.name === 'shape') {
        ...
        assert.notOk(
          shapeGroupKid.attrs.draggable,
          'Shape group \'shape\' must not be draggable.');
        assert.deepEqual(shapeGroupKid.attrs.x, 90,
          'Ellipse has the proper x.');
        assert.deepEqual(shapeGroupKid.attrs.y, 78,
          'Ellipse has the proper y.');
        assert.deepEqual(
