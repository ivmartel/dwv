...
QUnit.test("Test iterator.", function (assert) {
    ...
    var test0Max = 10;
    var i0Theo = test0Min;
    var iter0 = dwv.image.range(test0Min, test0Max);
    ...
    var test1Incr = 2;
    var i1Theo = test1Min;
    var iter1 = dwv.image.range(test1Min, test1Max, test1Incr);
...
QUnit.test("Test 3 components iterator.", function (assert) {
    ...
    var i0Theo = test0Min;
    var iter0 = dwv.image.range3d(test0Min, test0Max);
    var ival0 = iter0.next();
    while (!ival0.done) {
        assert.equal(ival0.value, i0Theo, "#0 3d iterator value");
        assert.equal(ival0.value1, i0Theo + 1, "#0 3d iterator value1");
        assert.equal(ival0.value2, i0Theo + 2, "#0 3d iterator value2");
        // increment
        i0Theo += 3;
        ival0 = iter0.next();
    }
    ...
    var test1Incr = 2;
    var i1Theo = test1Min;
    var iter1 = dwv.image.range3d(test1Min, test1Max, test1Incr);
    var ival1 = iter1.next();
    while (!ival1.done) {
        assert.equal(ival1.value, i1Theo, "#1 3d iterator value");
        assert.equal(ival1.value1, i1Theo + 1, "#1 3d iterator value1");
        assert.equal(ival1.value2, i1Theo + 2, "#1 3d iterator value2");
        i1Theo += test1Incr * 3;
        ival1 = iter1.next();
    }
    ...
    var i2Theo = test2Min;
    var iter2 = dwv.image.range3d(test2Min, test2Max, 1, true);
    var ival2 = iter2.next();
    while (!ival2.done) {
        assert.equal(ival2.value, i2Theo, "#2 3d iterator value");
        assert.equal(ival2.value1, i2Theo + test2Size, "#2 3d iterator value1");
        assert.equal(ival2.value2, i2Theo + 2 * test2Size, "#2 3d iterator value2");
        ++i2Theo;
        ival2 = iter2.next();
    }
    ...
    var test3Incr = 2;
    var i3Theo = test3Min;
    var iter3 = dwv.image.range3d(test3Min, test3Max, test3Incr, true);
    var ival3 = iter3.next();
    while (!ival3.done) {
        assert.equal(ival3.value, i3Theo, "#3 3d iterator value");
        assert.equal(ival3.value1, i3Theo + test3Size, "#3 3d iterator value1");
        assert.equal(ival3.value2, i3Theo + 2 * test3Size, "#3 3d iterator value2");
        i3Theo += test3Incr;
        ival3 = iter3.next();
    }
