...
describe('image', () => {
  ...
  test('VoiLutLinearFunction class', () => {
    ...
    assert.ok(Math.abs(f30.getY(5) - 127.5) < 1e-4,
      'VoiLutLinearFunction #31');
    assert.ok(Math.abs(f30.getY(9.5) - 255) < 1e-4,
      'VoiLutLinearFunction #32');
  });
  ...
  test('VoiLutSigmoidFunction class', () => {
    ...
    assert.ok(Math.abs(f00.getY(0) - 30.39) < 0.1,
      'VoiLutSigmoidFunction #00');
