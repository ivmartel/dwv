...
describe('utils', () => {
  ...
  test(
    ...
    () => {
      ...
      assert.equal(JSON.stringify(res10), JSON.stringify(ref10),
        'Split test10');
      // test11
      const test11 = 'root?key0=val00';
      const res11 = splitUri(test11);
      const ref11 = {base: 'root', query: {key0: 'val00'}};
      assert.equal(JSON.stringify(res11), JSON.stringify(ref11),
        'Split test11');
      ...
      assert.equal(JSON.stringify(res30), JSON.stringify(ref30),
        'Split test30');
      // test31
      ...
      assert.equal(JSON.stringify(res31), JSON.stringify(ref31),
        'Split test31');
   ...
   */
  test('getURIQuery - #DWV-REQ-IO-02-005 Window location URL scheme',
    () => {
      let params;
      ...
      assert.equal(
        res22.toString(), theo22.toString(),
        'Multiple key uri with three args');
      ...
      assert.equal(
        res23.toString(), theo23.toString(),
        'Multiple key uri with plenty args');
      ...
      assert.equal(res40.toString(), theo40.toString(),
        'Multiple file-like uri');
    }
  );
   ...
   *
   * @function module:tests/utils~parseWeasisXMLManifest
   */
